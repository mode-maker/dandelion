...
const ITEMS = [
  { title: "Лесное дыхание", img: "/ready/1.jpg", price: 1999, desc: "Природная гармония — свежесть зелени и лёгкость одуванчиков." },
  { title: "Тёплое лето",    img: "/ready/2.jpg", price: 1999, desc: "Яркие акценты природы, которые подарят уют и настроение дому." },
  { title: "Оазис уюта",     img: "/ready/3.jpg", price: 1999, desc: "Мини-сад, наполненный спокойствием и гармонией природы." },
  { title: "Светлый день",   img: "/ready/4.jpg", price: 1999, desc: "Живое украшение, создающее атмосферу тепла и вдохновения." },
  { title: "Дом солнца",     img: "/ready/5.jpg", price: 1999, desc: "Лаконичная композиция, наполняющая пространство светом и радостью." },
  { title: "Мягкое золото",  img: "/ready/6.jpg", price: 1999, desc: "Спокойные тёплые оттенки, которые создают мягкую, уютную атмосферу." },
];
...
export default function ReadyTerrariums() {
  ...
  return (
    // якорь + запас под фиксированный хедер
    <section id="ready" className="py-14 scroll-mt-24 lg:scroll-mt-28">
      <div className="max-w-[1200px] mx-auto px-6">
        {/* Заголовок блока */}
        <h2 className={`${caveat.className} text-[#ECEDE8] text-[32px] md:text-[36px] text-center tracking-wide mb-6`}>
          ГОТОВЫЕ ФЛОРАРИУМЫ
        </h2>
          ...
          <div className="rounded-2xl bg-[#E7E8E0] shadow-[0_10px_28px_rgba(0,0,0,0.25)] px-6 py-6">
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 z-10
                         rounded-full bg黑/35 hover:bg-black/50 text-white
                         backdrop-blur px-3 py-2 transition"
              title="Вперёд"
            >
              →
...
function Card({ title, img, desc, price }) {
  return (
    <article data-card className="snap-start flex-shrink-0 w-[320px] md:w-[340px] rounded-xl">
      {/* Заголовок */}
      ...
      <p className="mt-3 text-[16px] leading-7 text-[#333]">{desc}</p>
