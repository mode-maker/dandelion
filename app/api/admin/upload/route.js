...
// Авто-миграция: таблицы + нужные колонки
async function ensureSchema() {
  ...
  `;
  // на случай, если таблица уже была без этих колонок
...
export async function POST(request) {
  try {
    ...
    const { rows: chk } = await sql/* sql */`SELECT id FROM albums WHERE id = ${albumId} LIMIT 1`;
    if (!chk?.length) return NextResponse.json({ error: 'Альбом не найден' }, { status: 404 });
