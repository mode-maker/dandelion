...
export default function AdminAlbum({ params }) {
  ...
  const [items, setItems] = useState([]); // все фото (включая скрытые)
  ...
  // загрузка альбома + фото
  async function load() {
    const albums = await fetch('/api/admin/albums', { cache: 'no-store' })
      .then(r => r.json()).catch(() => []);
    setAlbum((albums || []).find(a => a.id === albumId) || null);

    const photos = await fetch(`/api/admin/photos?albumId=${albumId}&includeHidden=1`, { cache: 'no-store' })
      .then(r => r.json()).catch(() => []);
  ...
  async function togglePublish(it) {
    await fetch(`/api/admin/photos/${it.id}`, {
      method: 'PATCH',
      headers: {'Content-Type': 'application/json'},
  ...
  async function remove(it) {
    ...
    await fetch(`/api/admin/photos/${it.id}`, { method: 'DELETE' });
    await load();
  }

  async function makeCover(it) {
    // переносим выбранное фото в начало и сохраняем
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 md:py-12">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl md:text-3xl font-semibold text-[color:var(--aurora-3)]">
            Альбом: {album?.title || `#${albumId}`} {album?.event_date ? `• ${formatRuDate(album.event_date)}` : ''}
          </h1>
        ...
        {/* Превью альбома — как на публичной галерее */}
          ...
          <div className="px-0 pb-2">
            ...
            >
              ...
              {items.filter(x => x.published).length === 0 && (
                <div className="text-white/70 px-2 py-4">Нет опубликованных фото.</div>
              )}
            </div>
          </div>
            ...
            {items.map((it, i) => (
              <div key={it.id} className="w-full max-w-[520px] overflow-hidden rounded-2xl bg-black/10 ring-1 ring-white/5 shadow-lg">
                ...
                <div className="px-3 py-2 flex items-center justify-between">
                  {/* порядковый номер */}
                  <span className="px-2 py-0.5 rounded-lg text-xs bg-white/8 ring-1 ring-white/10">
                    {i + 1}
                  </span>
                  {/* статус */}
